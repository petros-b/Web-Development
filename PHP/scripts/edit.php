<?php 
//if user found in db and wants to change registration slot, prompt to user new option to choose from. Re-direct to update.php after 
session_start();
$id1=1;
$id2=2;
$id3=3;
...
try{
    ...
    //display to user current registration slot he/she has registered for

    $current_q="SELECT * FROM dates WHERE UMID=:s_user";
    $statement=$db->prepare($current_q);
    $statement->bindValue('s_user',$s_user);
    $statement->execute();
...
<body> 
    <main> 
       <h1>Please choose a registration time block:</h1> 
       <div id="date_sub">
<form action="" method="post" name="form1">
       <input type="radio" id="1" name="option" value="12/9/17 6:00PM to 7:00PM" <?php if(6-$rows['num']==0){echo "disabled";}?>>12/9/17, 6:00 PM – 7:00 PM<p>seats available: <?php echo 6-$rows['num'];?></p><br>
      ...
      <input type="radio" name="option" value="12/9/17 8:00PM to 9:00PM"<?php if(6-$rows3['num']==0){echo "disabled";}?>>12/9/17, 8:00 PM – 9:00 PM<p>seats available: <?php echo 6- $rows3['num'];?></p><br>
       ...
       <input type="radio" name="option" value="12/10/17 6:00PM to 7:00PM"<?php if(6-$rows4['num']==0){echo "disabled";}?>>12/10/17, 6:00 PM – 7:00 PM<p>seats available: <?php echo 6- $rows4['num'];?></p><br>
